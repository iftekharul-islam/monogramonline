        ...
        <tr>
          ...
          <td>
            ...
            @foreach($order->notes as $note)
              @if(strpos(strtolower($note->note_text), 'address verification'))
                @if($emails == 0)
            ...
            <button type = "button" class = "btn btn-link" data-toggle = "modal"
                    ...
                    onclick="open_email({{ $order->id }},'{{ $order->short_order }}','{{ $order->customer->bill_email }}', 8)">
          ...
          <td>
            ...
            {!! Form::hidden('tab', 'address') !!}
            {!! Form::hidden('order_5p', $order->id) !!}
            {!! Form::submit('Release Address Hold', ['class' => 'btn btn-xs btn-info']) !!}
            {!! Form::close() !!}
            <br>
            {!! Form::open(['class' => 'ajax_form', 'method' => 'get']) !!}
            {!! Form::hidden('tab', 'address') !!}
            ...
            {!! Form::submit('Ignore Validation', ['class' => 'btn btn-xs btn-warning']) !!}
            {!! Form::close() !!}
          </td>
        </tr>
        @endforeach
...
@else
    <div class = "alert alert-warning">No Address Holds.</div>
@endif