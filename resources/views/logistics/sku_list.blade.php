						...
						<div class = "form-group col-xs-5">
							<select name="stock_select"  id="stock_select" class="my-select" style="width:500px;">
								...
								@foreach ($stock_no_list as $stock)
									<option data-img-src="{{ $stock->warehouse }}" value="{{ $stock->stock_no_unique }}">{{ $stock->description }}</option> 
								@endforeach
							</select>
						</div>
									...
									<td rowspan=2 width="110px" class="info">
										...
										@if ($option->product)
											| <a href = "{{ $option->product->product_url }}"
												data-toggle = "tooltip" data-placement = "top"
												title = "View on Web" 
												target = "_blank"><i class = 'glyphicon glyphicon-picture text-primary'></i></a>
										@endif
								...
								<tr>
													...
													<tr>
														<td rowspan=2 width="50px">
															@if ($inventoryunit->inventory && $inventoryunit->inventory->warehouse != '')
																<img src = "{{ $inventoryunit->inventory->warehouse }}" height="40">
															@endif
														</td>
														<td>
									...
									<td width="150px"> 
										...
										@if ($option->graphic_sku == 'NeedGraphicFile' || $option->graphic_sku == '')
											 hidden
										@endif
										" id="CNF-{{ $option->unique_row_value }}" name="{{ $option->graphic_sku }}">Configure Graphic SKU</a>
									</div>
									</td>
									<td width="175px"> 
										@if ($option->design)
