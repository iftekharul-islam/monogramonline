...
class Batch extends Taskable
{
    ...
    public static function getNextStation ($type, $batch_route_id, $current_station_id)
    {
      ...
      $next_stations = DB::select(sprintf("SELECT * FROM batch_route_station 
            WHERE batch_route_id = %d and id > ( 
                SELECT id FROM batch_route_station 
                  WHERE batch_route_id = %d AND station_id = %d)", 
            $batch_route_id, $batch_route_id, $current_station_id));
    ...
    {      
      $prev_stations = DB::select(sprintf("SELECT * FROM batch_route_station 
            WHERE batch_route_id = %d and id < ( 
                SELECT id FROM batch_route_station 
                  WHERE batch_route_id = %d AND station_id = %d)
                  order by id DESC", 
    ...
    public static function readyToExport() {
      ...
      foreach ($batches as $batch) {
        foreach ($batch->pending_items  as $item) {
          ...
          if (in_array($item->parameter_option->graphic_sku, $stylenames) && 
