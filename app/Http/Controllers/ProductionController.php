...
class ProductionController extends Controller
{
    public function status(Request $request)
    {
        ...
        return view('production.status',
            compact('stations', 'stations_status', 'sections', 'user_section', 'late', 'very_late'));
    }
    ...
    public function statusDetail(Request $request)
    {
        ...
        return view('production.status_detail',
            compact('station', 'station_name', 'ready', 'in_progress', 'activity', 'next_in_route', 'stations',
    ...
    public function scanWork(ScanRequest $request)
    {
                ...
                if(count((array)$scan) == 0) {
                    ...
                    if($batch->section->start_finish == '0') {
                        ...
                        if($next_station == null) {
                            ...
                            $messages[] = [
                                'Moved to:',
                                $next_station->station_name . ' - ' . $next_station->station_description
                            ];
        ...
        if(count((array)$batch) > 0) {
            $stations = BatchRoute::routeThroughStations($batch->batch_route_id,
                Station::find($batch->station_id)->station_name);
        } else {
            $stations = null;
        }
    ...
    public function openMoveNext()
    {
        ...
        return view('production.moveNext',
            compact('batches', 'station', 'route', 'stations_list', 'scan_batches', 'scan_batches_image',
...
//        dd($dst);
        ...
        if($request->has('scan_batches') && $request->get('scan_batches') != ',') {
            ...
            foreach ($batch_array as $batch_number) {
                ...
                if(!$batch) {
                    ...
                    if($related) {
                        $batch = $related;
                        $success[] = sprintf('Batch %s selected, Batch %s is inactive <br>', $batch->batch_number,
                            $batch_number);
                    } else {
            ...
            if($request->has('vendor') && $request->get('vendor') != '') {
            ...
            $batches = Batch::with('first_item.order', 'route', 'itemsCount')
                ...
                if($vendor != '') {
                    $batches->whereHas('first_item', function ($q) use ($vendor) {
                        $q->where('vendor', $vendor);
                    });
                }
            $batches = $batches->searchRoute($request->get('route'))
                ->searchStatus('movable')
                //->where('stations.type', 'P')
    ...
    public function moveTonextStation($batchNumber)
    {

        $batch_update = Batch::with('route', 'station')
            ->whereIn('batch_number', $batchNumber)
            ->get();

        foreach ($batch_update as $batch) {
            $next_station = Batch::getNextStation('object', $batch->batch_route_id, $batch->station_id);

            if($batch->station->station_name != "S-Wait_for_Pic") {
                return false;
                break;
            }
            ...
            if($next_station && $next_station->id != '0') {
                ...
                $success = sprintf('Batch %s *Successfully Moved to %s<br>', $batch->batch_number,
                    $next_station->station_name);
//                Batch::note($batch->batch_number, $next_station->id, '0', $success);
                return true;
            } else {
                $error = sprintf('Batch %s has no ***further stations on route <br>', $batch->batch_number);
//                Batch::note($batch->batch_number, $next_station->id, '0', $error);
                return false;
            }
        }
        return false;
