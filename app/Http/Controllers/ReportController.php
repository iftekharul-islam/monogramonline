...
class ReportController extends Controller
{
	...
	public function section(Request $request)
	{
        ...
        if(empty($vendor)){
            $vendor =  !empty($request->get('vendor')) ? $request->get('vendor') : null ;
        }
...
//            ->whereNULL('items.vendor')
			->selectRaw("
                            items.manufacture_id,
                            SUM(items.item_quantity) as items_count,
                            count(items.id) as lines_count,
                            CAST(SUM(CASE WHEN items.vendor = 'VENDOR-A' THEN 1 ELSE 0 END) AS UNSIGNED) as vendor_A_count,
                            CAST(SUM(CASE WHEN items.vendor = 'VENDOR-B' THEN 1 ELSE 0 END) AS UNSIGNED) as vendor_B_count,
                            CAST(SUM(CASE WHEN items.vendor = 'VENDOR-C' THEN 1 ELSE 0 END) AS UNSIGNED) as vendor_C_count,
        ...
        $vendor_items = Item::join('batches', 'batches.batch_number', '=', 'items.batch_number')
            ->join('orders', 'items.order_5p', '=', 'orders.id')
            ->join('stations', 'batches.station_id', '=', 'stations.id')
            ->join('sections', 'stations.section', '=', 'sections.id')
            ->searchStore($store_ids)
...
//        dd($vendor_items);
//
//        $vendor_a = $items->where('vendor', 'VENDOR-A');
//        $vendor_b = $items->where('vendor', 'VENDOR-B');
//        $vendor_c = $items->where('vendor', 'VENDOR-C');
//        dd($vendor_a, $vendor_b, $vendor_c);
		...
		//   dd(['manufactures'=>$manufactures, 'items'=> $items, 'qc'=>$qc, 'backorders'=>$backorders, 'rejects'=>$rejects, 'WAP'=>$WAP, 'unbatched'=>$unbatched, 'shipped_today'=>$shipped_today, 'rejected_today'=>$rejected_today, 'date'=>$date, 'section'=>$section, 'graphic_statuses'=>$graphic_statuses, 'now'=>$now, 'CS'=>$CS, 'order_statuses'=>$order_statuses, 'CS_rejects'=>$CS_rejects, 'total'=>$total, 'stores'=>$stores, 'store_ids'=>$store_ids, 'store_link'=>$store_link, 'max_date'=>$max_date, 'batch_type'=>$batch_type, 'sections'=>$sections, 'section_totals'=>$section_totals ]);
		return view('reports.section', compact(
			'manufactures',
			'items',
			'qc',
			'backorders',
	...
	public function sectionFilter(Request $request)
	{
		...
		return view('reports.sectionFilter', compact(
			'items',
			'qc',
			'backorders',
			'rejects',
	...
	public function stationSummary(Request $request)
	{
		...
		return view('reports.station_summary', compact(
			'summaries',
			'total_lines',
			'total_items',
			'stations_arrays',
	...
	public function orderDateReport(Request $request)
	{
		...
		return view('reports.order_date', compact(
			'ordered_today',
			'start_date',
			'end_date',
			'stores',
	...
	public function shipDateReport(Request $request)
	{
		...
		return view('reports.ship_date', compact(
			'shipped_today',
			'start_date',
			'end_date',
			'stores',
	...
	public function itemsReport(Request $request)
	{
		...
		return view('reports.items', compact(
			'items',
			'rejects',
			'start_date',
			'end_date',
